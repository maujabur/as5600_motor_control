...
function render(s){const phases={STOPPED:'EM REPOUSO',TO_END:'INDO AO FIM',DWELL_END:'PAUSA NO FIM',TO_START:'INDO AO INÍCIO',DWELL_START:'PAUSA NO INÍCIO'};last={start:s.start,end:s.end,angle:s.angle||0};$('phase').textContent=s.moveActive&&!s.running?'AJUSTANDO POSIÇÃO':(phases[s.phase]||s.phase);$('angle').textContent=s.sensor?s.angle.toFixed(1)+'°':'SEM SENSOR';$('run').disabled=s.running||s.moveActive||s.otaBusy;$('stop').disabled=!s.running&&!s.moveActive;const canAdjust=!s.running&&!s.moveActive&&!s.otaBusy&&s.sensor;$('goStart').disabled=!canAdjust;$('goEnd').disabled=!canAdjust;$('ota').textContent=s.otaBusy?'OTA/AP: UPDATE':'OTA/AP: IDLE';if(!editing){$('start').value=s.start.toFixed(1);$('end').value=s.end.toFixed(1);$('rpmOut').value=s.rpmOut.toFixed(2);$('rpmBack').value=s.rpmBack.toFixed(2);$('waitStart').value=s.waitStart;$('waitEnd').value=s.waitEnd;$('rpmOut').max=s.maxRpm;$('rpmBack').max=s.maxRpm}drawGauge()}
