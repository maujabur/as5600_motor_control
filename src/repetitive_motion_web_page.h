...
function render(s){const unit=String(s.unit).padStart(2,'0'),phases={STOPPED:'EM REPOUSO',TO_END:'INDO AO FIM',DWELL_END:'PAUSA NO FIM',TO_START:'INDO AO INÍCIO',DWELL_START:'PAUSA NO INÍCIO'};$('unitLabel').textContent='MOTION UNIT '+unit;document.title='Motion Unit '+unit;last={start:s.start,end:s.end,angle:s.angle||0};$('phase').textContent=s.stall?'FALHA: MOTOR TRAVADO':(s.moveActive&&!s.running?'MOVIMENTO AVULSO':(phases[s.phase]||s.phase));$('angle').textContent=s.sensor?s.angle.toFixed(1)+'°':'SEM SENSOR';$('run').disabled=s.running||s.moveActive||s.otaBusy;$('stop').disabled=!s.running&&!s.moveActive;const canAdjust=!s.running&&!s.moveActive&&!s.otaBusy&&s.sensor;$('goStart').disabled=!canAdjust;$('goEnd').disabled=!canAdjust;$('manualGo').disabled=!canAdjust;$('manualTarget').disabled=!canAdjust;$('manualRpm').disabled=!canAdjust;$('manualRpm').max=s.maxRpm;$('ota').textContent=s.otaBusy?'OTA/AP: UPDATE':'OTA/AP: IDLE';if(!editing){$('start').value=s.start.toFixed(1);$('end').value=s.end.toFixed(1);$('rpmOut').value=s.rpmOut.toFixed(2);$('rpmBack').value=s.rpmBack.toFixed(2);$('waitStart').value=s.waitStart;$('waitEnd').value=s.waitEnd;$('rpmOut').max=s.maxRpm;$('rpmBack').max=s.maxRpm}drawGauge()}
